...
ground state일때는 빛을 내지 않지만 외부에서 빛이 형광 분자를 hit 하면 분자는 빛 에너지를 흡수할 수 있음 -> 이 프로세스를 "excitation"이라고 함

Fluorophore가 high-energy configuration 일때는 unstable하기때문에 결국 lowest-energy excited state를 취합니다.semi-stable이라고 부르는
